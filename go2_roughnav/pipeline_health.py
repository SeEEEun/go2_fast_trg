#!/usr/bin/env python3
"""Print a compact health report for the Isaac Gym rough navigation pipeline."""
...
class PipelineHealth(Node):
    def __init__(self):
        ...
        self.create_subscription(PointCloud2, "/lidar/points", lambda m: self._cloud("/lidar/points", m), 10)
        self.create_subscription(Imu, "/imu/data", lambda m: self._touch("/imu/data", f"az={m.linear_acceleration.z:.2f}"), 10)
    ...
    def _odom(self, msg: Odometry) -> None:
        ...
        self._touch("/Odometry", f"x={p.x:.2f}, y={p.y:.2f}")
